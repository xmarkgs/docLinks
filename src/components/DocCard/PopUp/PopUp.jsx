                    ...
                    <div className="popup__links">
                        <div className="popup__links__column">
                            ...
                            {doc.links.foundLinks.map((link) => (
                                <a key={link.link} href={link.link} className="popup__link">{link.linkTitle}</a>
                            ))}
                        </div>
                        <div className="popup__links__column">
                            <h4 className="popup__links__title">Found in files:</h4>
                            {doc.links.matchedLinksInOtherFiles.map((item) => (
                                <a key={item.foundInDocID} href={`https://docs.google.com/document/d/${item.foundInDocID}`} className="popup__link">{item.foundInDocTitle}</a>
                            ))}
                        </div>
                    </div>
                </div>
